...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('codes', nargs='*', type=str, help='종목코드들 (공백으로 구분, 비워두면 모든 종목 업데이트)')
    ...
    def handle(self, *args, **options):
        ...
        if not codes:
            self.stdout.write('종목 코드가 입력되지 않아, 저장된 모든 종목을 업데이트합니다.')
            # 저장된 모든 Stock 객체에서 코드만 리스트로 추출
            codes = [s.code for s in Stock.objects.all()]

            if not codes:
                self.stdout.write(self.style.WARNING('저장된 종목이 없습니다. 코드를 입력해서 실행해주세요.'))
                return
